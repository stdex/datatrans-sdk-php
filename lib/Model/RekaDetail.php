 ...
 */
class RekaDetail implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'RekaDetail';
    ...
    const SUBTYPE_REK = 'REK';
    const SUBTYPE_REL = 'REL';
    const SUBTYPE_RER = 'RER';
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['subtype']) && (mb_strlen($this->container['subtype']) > 3)) {
            $invalidProperties[] = "invalid value for 'subtype', the character length must be smaller than or equal to 3.";
        }

        if (!is_null($this->container['subtype']) && (mb_strlen($this->container['subtype']) < 3)) {
            $invalidProperties[] = "invalid value for 'subtype', the character length must be bigger than or equal to 3.";
        }
     ...
     * Sets masked
     *
     * @param string|null $masked Masked REKA number. Can be used to display on a users profile page.
     *
     * @return self
     */
     ...
     * Sets expiry_month
     *
     * @param string|null $expiry_month The expiry month of the REKA card
     *
     * @return self
     */
    public function setExpiryMonth($expiry_month)
    {
        if (!is_null($expiry_month) && (mb_strlen($expiry_month) > 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_month when calling RekaDetail., must be smaller than or equal to 2.');
        }
        if (!is_null($expiry_month) && (mb_strlen($expiry_month) < 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_month when calling RekaDetail., must be bigger than or equal to 2.');
        }
        if (!is_null($expiry_month) && (!preg_match("/\\d{2}/", $expiry_month))) {
            throw new \InvalidArgumentException("invalid value for $expiry_month when calling RekaDetail., must conform to the pattern /\\d{2}/.");
        }
     ...
     * Sets expiry_year
     *
     * @param string|null $expiry_year The expiry year of the REKA card
     *
     * @return self
     */
    public function setExpiryYear($expiry_year)
    {
        if (!is_null($expiry_year) && (mb_strlen($expiry_year) > 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_year when calling RekaDetail., must be smaller than or equal to 2.');
        }
        if (!is_null($expiry_year) && (mb_strlen($expiry_year) < 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_year when calling RekaDetail., must be bigger than or equal to 2.');
        }
        if (!is_null($expiry_year) && (!preg_match("/\\d{2}/", $expiry_year))) {
            throw new \InvalidArgumentException("invalid value for $expiry_year when calling RekaDetail., must conform to the pattern /\\d{2}/.");
        }
     ...
     * Sets subtype
     *
     * @param string|null $subtype The REKA payment subtype. REK for REKA Check, REL for REKA Lunch, RER for REKA Rail.
     *
     * @return self
     */
    public function setSubtype($subtype)
    {
        ...
        if (!is_null($subtype) && (mb_strlen($subtype) > 3)) {
            throw new \InvalidArgumentException('invalid length for $subtype when calling RekaDetail., must be smaller than or equal to 3.');
        }
        if (!is_null($subtype) && (mb_strlen($subtype) < 3)) {
            throw new \InvalidArgumentException('invalid length for $subtype when calling RekaDetail., must be bigger than or equal to 3.');
        }

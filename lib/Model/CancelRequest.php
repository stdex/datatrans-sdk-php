...
class CancelRequest implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setRefno($refno)
    {
        if ((mb_strlen($refno) > 20)) {
            throw new \InvalidArgumentException('invalid length for $refno when calling CancelRequest., must be smaller than or equal to 20.');
        }
        if ((mb_strlen($refno) < 1)) {
            throw new \InvalidArgumentException('invalid length for $refno when calling CancelRequest., must be bigger than or equal to 1.');
        }

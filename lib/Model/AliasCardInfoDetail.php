...
/**
 * AliasCardInfoDetail Class Doc Comment
 *
 * @category Class
 * @package  Datatrans\Client
 * @author   OpenAPI Generator team
 ...
 */
class AliasCardInfoDetail implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'AliasCardInfoDetail';
    ...
    protected static $openAPITypes = [
        'expiry_month' => 'string',
        'expiry_year' => 'string',
        'card_info' => '\Datatrans\Client\Model\CardInfo'
    ];
    ...
    public function __construct(array $data = null)
    {
        ...
        $this->container['card_info'] = $data['card_info'] ?? null;
    }
     ...
     * Sets expiry_month
     *
     * @param string|null $expiry_month The expiry month of the card. Currently not available when the Convert API was used before.
     *
     * @return self
     */
    public function setExpiryMonth($expiry_month)
    {
        if (!is_null($expiry_month) && (mb_strlen($expiry_month) > 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_month when calling AliasCardInfoDetail., must be smaller than or equal to 2.');
        }
        if (!is_null($expiry_month) && (mb_strlen($expiry_month) < 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_month when calling AliasCardInfoDetail., must be bigger than or equal to 2.');
        }
        if (!is_null($expiry_month) && (!preg_match("/\\d{2}/", $expiry_month))) {
            throw new \InvalidArgumentException("invalid value for $expiry_month when calling AliasCardInfoDetail., must conform to the pattern /\\d{2}/.");
        }
     ...
     * Sets expiry_year
     *
     * @param string|null $expiry_year The expiry year of the card. Currently not available when the Convert API was used before.
     *
     * @return self
     */
    public function setExpiryYear($expiry_year)
    {
        if (!is_null($expiry_year) && (mb_strlen($expiry_year) > 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_year when calling AliasCardInfoDetail., must be smaller than or equal to 2.');
        }
        if (!is_null($expiry_year) && (mb_strlen($expiry_year) < 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_year when calling AliasCardInfoDetail., must be bigger than or equal to 2.');
        }
        if (!is_null($expiry_year) && (!preg_match("/\\d{2}/", $expiry_year))) {
            throw new \InvalidArgumentException("invalid value for $expiry_year when calling AliasCardInfoDetail., must conform to the pattern /\\d{2}/.");
        }
     ...
     * Sets card_info
     *
     * @param \Datatrans\Client\Model\CardInfo|null $card_info card_info
     *
     * @return self
     */

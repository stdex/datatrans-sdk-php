...
/**
 * CardValidateRequest Class Doc Comment
 *
 * @category Class
 ...
 */
class CardValidateRequest implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'CardValidateRequest';
    ...
    public function setExpiryMonth($expiry_month)
    {
        if (!is_null($expiry_month) && (mb_strlen($expiry_month) > 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_month when calling CardValidateRequest., must be smaller than or equal to 2.');
        }
        if (!is_null($expiry_month) && (mb_strlen($expiry_month) < 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_month when calling CardValidateRequest., must be bigger than or equal to 2.');
        }
        if (!is_null($expiry_month) && (!preg_match("/\\d{2}/", $expiry_month))) {
            throw new \InvalidArgumentException("invalid value for $expiry_month when calling CardValidateRequest., must conform to the pattern /\\d{2}/.");
        }
    ...
    public function setExpiryYear($expiry_year)
    {
        if (!is_null($expiry_year) && (mb_strlen($expiry_year) > 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_year when calling CardValidateRequest., must be smaller than or equal to 2.');
        }
        if (!is_null($expiry_year) && (mb_strlen($expiry_year) < 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_year when calling CardValidateRequest., must be bigger than or equal to 2.');
        }
        if (!is_null($expiry_year) && (!preg_match("/\\d{2}/", $expiry_year))) {
            throw new \InvalidArgumentException("invalid value for $expiry_year when calling CardValidateRequest., must conform to the pattern /\\d{2}/.");
        }

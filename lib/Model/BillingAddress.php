...
class BillingAddress implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setGender($gender)
    {
        if (!is_null($gender) && (mb_strlen($gender) > 6)) {
            throw new \InvalidArgumentException('invalid length for $gender when calling BillingAddress., must be smaller than or equal to 6.');
        }
        if (!is_null($gender) && (!preg_match("/^([a-zA-Z]){2,6}$/", $gender))) {
            throw new \InvalidArgumentException("invalid value for $gender when calling BillingAddress., must conform to the pattern /^([a-zA-Z]){2,6}$/.");
        }
     ...
     * Sets phone_number
     *
     * @param string|null $phone_number Phone number of the person
     *
     * @return self
     */

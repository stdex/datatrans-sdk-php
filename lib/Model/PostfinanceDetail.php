...
/**
 * PostfinanceDetail Class Doc Comment
 *
 * @category Class
 ...
 */
class PostfinanceDetail implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'PostfinanceDetail';
     ...
     * Sets masked
     *
     * @param string|null $masked Masked card number. Can be used to display on a users profile page. For example: `****2345`
     *
     * @return self
     */
     ...
     * Sets expiry_month
     *
     * @param string|null $expiry_month The expiry month of the Postfinance card
     *
     * @return self
     */
    public function setExpiryMonth($expiry_month)
    {
        if (!is_null($expiry_month) && (mb_strlen($expiry_month) > 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_month when calling PostfinanceDetail., must be smaller than or equal to 2.');
        }
        if (!is_null($expiry_month) && (mb_strlen($expiry_month) < 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_month when calling PostfinanceDetail., must be bigger than or equal to 2.');
        }
        if (!is_null($expiry_month) && (!preg_match("/\\d{2}/", $expiry_month))) {
            throw new \InvalidArgumentException("invalid value for $expiry_month when calling PostfinanceDetail., must conform to the pattern /\\d{2}/.");
        }
     ...
     * Sets expiry_year
     *
     * @param string|null $expiry_year The expiry year of the Postfinance card
     *
     * @return self
     */
    public function setExpiryYear($expiry_year)
    {
        if (!is_null($expiry_year) && (mb_strlen($expiry_year) > 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_year when calling PostfinanceDetail., must be smaller than or equal to 2.');
        }
        if (!is_null($expiry_year) && (mb_strlen($expiry_year) < 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_year when calling PostfinanceDetail., must be bigger than or equal to 2.');
        }
        if (!is_null($expiry_year) && (!preg_match("/\\d{2}/", $expiry_year))) {
            throw new \InvalidArgumentException("invalid value for $expiry_year when calling PostfinanceDetail., must conform to the pattern /\\d{2}/.");
        }

...
class MerchantData implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    const WHITE_LIST_STATUS_Y = 'Y';
    const WHITE_LIST_STATUS_N = 'N';
     ...
     * @return string[]
     */
    public function getWhiteListStatusAllowableValues()
    {
        return [
            self::WHITE_LIST_STATUS_Y,
            self::WHITE_LIST_STATUS_N,
        ];
    }
    ...
    {
        $this->container['white_list_status'] = $data['white_list_status'] ?? null;
    }
    ...
    public function listInvalidProperties()
    {
        ...
        $allowedValues = $this->getWhiteListStatusAllowableValues();
        if (!is_null($this->container['white_list_status']) && !in_array($this->container['white_list_status'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value '%s' for 'white_list_status', must be one of '%s'",
                $this->container['white_list_status'],
     ...
     *
     * @param string|null $white_list_status white_list_status
     *
     * @return self
     */
    ...
    {
        $allowedValues = $this->getWhiteListStatusAllowableValues();
        if (!is_null($white_list_status) && !in_array($white_list_status, $allowedValues, true)) {

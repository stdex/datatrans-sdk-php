...
/**
 * AliasConvertRequest Class Doc Comment
 *
 * @category Class
 * @package  Datatrans\Client
 * @author   OpenAPI Generator team
 ...
 */
class AliasConvertRequest implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'AliasConvertRequest';
    ...
    {
        $this->container['legacy_alias'] = $data['legacy_alias'] ?? null;
    ...
    public function listInvalidProperties()
    {
        ...
        if ($this->container['legacy_alias'] === null) {
            $invalidProperties[] = "'legacy_alias' can't be null";
        }
     ...
     * @return string
     */
    public function getLegacyAlias()
    {
        return $this->container['legacy_alias'];
    }

    /**
     * Sets legacy_alias
     *
     * @param string $legacy_alias The legacy alias
     *
     * @return self
     */
    public function setLegacyAlias($legacy_alias)
    {
        $this->container['legacy_alias'] = $legacy_alias;
     ...
     * Sets expiry_month
     *
     * @param string|null $expiry_month The expiry month of the credit card behind alias.
     *
     * @return self
     */
    public function setExpiryMonth($expiry_month)
    {
        if (!is_null($expiry_month) && (mb_strlen($expiry_month) > 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_month when calling AliasConvertRequest., must be smaller than or equal to 2.');
        }
        if (!is_null($expiry_month) && (mb_strlen($expiry_month) < 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_month when calling AliasConvertRequest., must be bigger than or equal to 2.');
        }
        if (!is_null($expiry_month) && (!preg_match("/\\d{2}/", $expiry_month))) {
            throw new \InvalidArgumentException("invalid value for $expiry_month when calling AliasConvertRequest., must conform to the pattern /\\d{2}/.");
        }
     ...
     * Sets expiry_year
     *
     * @param string|null $expiry_year The expiry year of the credit card behind the alias
     *
     * @return self
     */
    public function setExpiryYear($expiry_year)
    {
        if (!is_null($expiry_year) && (mb_strlen($expiry_year) > 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_year when calling AliasConvertRequest., must be smaller than or equal to 2.');
        }
        if (!is_null($expiry_year) && (mb_strlen($expiry_year) < 2)) {
            throw new \InvalidArgumentException('invalid length for $expiry_year when calling AliasConvertRequest., must be bigger than or equal to 2.');
        }
        if (!is_null($expiry_year) && (!preg_match("/\\d{2}/", $expiry_year))) {
            throw new \InvalidArgumentException("invalid value for $expiry_year when calling AliasConvertRequest., must conform to the pattern /\\d{2}/.");
        }

 ...
 */
class SuperCard implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'SuperCard';
    ...
    const SUBTYPE_SCM = 'SCM';
    const SUBTYPE_SCV = 'SCV';
    const SUBTYPE_SCP = 'SCP';
     ...
     * Sets subtype
     *
     * @param string|null $subtype The SCX payment subtype. SCM for Mastercard, SCV for Visa and SCP for Visa prepaid
     *
     * @return self
     */
    public function setSubtype($subtype)
    {
        ...
        if (!is_null($subtype) && (mb_strlen($subtype) > 3)) {
            throw new \InvalidArgumentException('invalid length for $subtype when calling SuperCard., must be smaller than or equal to 3.');
        }
        if (!is_null($subtype) && (mb_strlen($subtype) < 3)) {
            throw new \InvalidArgumentException('invalid length for $subtype when calling SuperCard., must be bigger than or equal to 3.');
        }
